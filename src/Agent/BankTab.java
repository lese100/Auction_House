...
/**
 * Used to create a "bank" Tab that will be displayed by the display. This would
 * allow for future implementation of multiple banks. The object also handles
 * events pertaining to its lists and has getter and setters for these lists.
...
public class BankTab {
    ...
    /**
     * Creates a new bank tab format, places the provides buttons. Does all the
     * formatting for the tab and stores it inside bankTab. Adds a event
     * handlers for selecting things from the listView.
     * @param leave the leave button for the bank
     * @param getAuctions button used to request a list of auction houses from
     *                    the bank
     * @param getBalance button used to request the users balance from the bank
     * @param transfer button used to send a message to the bank to transfer
     *                 funds
     * @param join button used to join auction houses from the list of auction
     *             houses
     * @param deposit initial deposit of the user to the bank.
     */
    public BankTab(Button leave,Button getAuctions, Button getBalance,
        ...
        /*
         * formatting for the Pending Transfers , the button to transfer funds,
         * and the Purchase history list
         */
        VBox transactions = new VBox();
        VBox transferHold = new VBox();
        transferHold.getChildren().addAll(pendingTransactions,pending,transfer);
        transactions.getChildren().addAll(transferHold,purchaseHistory,
                purchases);
        transactions.setSpacing(10);
        purchasedDisp.add("Deposit" + getSpacing("Deposit") + "+$" +
                df.format(deposit));
        ...
        /*
         * sets the style and default text for the labels of the user account
         * information
         */
        name = new Label("  name");
        ...
        /*
         * formats the labels and buttons used for displaying user account
         * information
         */
        ...
        /*
         * places everything accordingly on a boarder pane and adds it to the
         * Tab
         */
        ...
        /*
         * Event handler for the pendingTransfer list. Sets the current Selected
         * Item
         */
        ...
        /*
         * Event Handler for the list of Auction Houses. Sets the current
         * selected item.
         */
    ...
    /**
     * gets the IDRecord of the currently selected auction house in the List of
     * auction houses
    ...
    /**
     * creates spacing to help formatting the transfer and purchase history
     * listView.
    ...
    /**
     * Updates the Pending list of transfers with the current ArrayList Storing
     * them.
     */
    private void updatePending(){
        ...
        for(AuctionItem item : pendingTransfer){
            ...
            info += getSpacing(info) + "-$" + df.format(item.getBid().
                    getCurrentBid());
    ...
    public AuctionItem getSelectedToTransfer(){
        ...
        info += getSpacing(info) + "-$" + df.format(hold.getBid().
                getCurrentBid());
    ...
    /**
     * adds a item to the list of items that need funds transferred and updates
     * the display.
    ...
    /**
     * Updates the labels that hold the users balance, username, unfrozen funds,
     * and account number. Called mainly whenever someone requests getBalance
     * from the bank.
     * @param account BankAccount object that holds my balance, unfrozen funds,
     *                name, and account number.
     */
    public void updateLabels(BankAccount account){
        name.setText("  "+account.getUserName());
        accountNum.setText("    Account#: " + Integer.toString(account.
                getAccountNumber()));
        frozenBal.setText(" (" + df.format(account.getTotalUnfrozen()) + ")");
        totalBal.setText("    Balance: " + df.format(account.
                getTotalBalance()));
    }
}
