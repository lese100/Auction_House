...
accounts with the Bank, which then serves as a conduit for monetary
...
manifestations of both Agents and Auction Houses. 
...
account information for the Bank. And example of this interface is shown
below (or see the file `BankGUI01.png` in the src/resource folder if the
image does not appear):

![Display Screen Shot](./src/resource/BankGUI02.png)

Along the left-hand side you will see the
designated name (at the top), the hostname, and the port number being used,
...
receive a warning and be unable to close the Bank of there are active
accounts.

**(2) Starting an Auction House (AH).** In the process of starting up an
AH, the AH automatically connects to the Bank. Thus
you will want to start up the Bank first. With the Bank already running,
starting up an AH presents you with a GUI like that shown below
(or see the file `AuctionHouseGUI01.png` in the src/resource folder if the
image does not appear):

![Display Screen Shot](./src/resource/AuctionHouseGUI.png)

Across the top you will see the AH name and a unique,
randomly-generated integer ID number (which corresponds to the AH's
bank account number), the AH's current bank balance (which will
be $0 when first started), and the amount owed the AH from Agents
who have won bids on auction items.

In the bottom pane you will seem some brief information describing
connection activity, beginning with the AH's connection to the Bank, and
giving details about Agents connecting to the AH.

In the center pane you will see a table listing the items up for auction at
this AH. The STATE column tells you if bidding is OPEN (but no bids received
yet), BIDDING (meaning at least one bid has been received), or CLOSED (meaning
the item has sold). Once a bid has been accepted by the AH as valid, the
Current Bid column reflects the newly accepted bid and a bid
...
the item is marked as SOLD to the bidding Agent, the line item turns red,
and an alert is sent along to the Agent. The winning bid amount is added to
the "Amount Owed" along the top.
...
starting up an Agent elicits an initial GUI demanding information about
...
The user is then presented with a GUI like that shown below (or see the file
...
balance (the full balance, plus the un-frozen portion of the full balance).

**(4)** Agent Participating in an Auction.  On the Bank tab, you can
select the `Ask For Auctions` button and receive a list (from the Bank) of
...
which displays the auction items available from that Auction House. Selecting
an item from that list of auction items transfers some of the information
about that item over to the right-hand pane and allows you to submit a bid
on that item. If there is an image available for the item, the image will be
displayed. An example Auction House tab is shown in the image below (or see
the file `AgentGUI4.png` in the src/resource folder if the image does not
appear):
